...
local radius = 2.5 -- not correctly working!
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
    ...
    bucurrentFuel = currentFuel
	-- Get the gas price
	
	while nearStation == nil do
		Citizen.Wait(200)
	end

	local extraCost = nearStation.fuelcost
	--print('Fuelcost: ' .. extraCost)
	local stationTank = nearStation.fuel
	local pumpedFuel = 0

	while isFueling do
		Citizen.Wait(500)
		pumpedFuel = pumpedFuel + 1
		...
		if((stationTank - pumpedFuel) == 0) then
			isFueling = false
			ESX.ShowNotification('~r~Die Tankstelle hat kein Benzin mehr!')
		end
	...
	if pumpObject then
        ...
        TriggerServerEvent('igs:updateSoldFuel', nearStation.id, (currentFuel - bucurrentFuel))
        TriggerServerEvent('igs:updateFuel', nearStation.id, (currentFuel - bucurrentFuel))
        TriggerServerEvent('igs:updateAccount', nearStation.id, currentCost)

        -- Remove fuel from station
        -- Add account to station
        -- Add soldfuel to station + 
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			...
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped), -1) == ped then
				...
				if(registeredVehicle) then

					DrawText3Ds(pumpCoords.x, pumpCoords.y, pumpCoords.z + 1.2, "Druecke E um deinen Tankinhalt(" .. fuelInside .. ") fuer $" .. nearStation.buyprice .. " /Liter zu verkaufen.")
					...
					if(IsControlJustReleased(0, 38)) then
						...
						if(not selling) then
							...
							stationBalance = nearStation.account
							maxLitres = fuelInside
							litrePrice = nearStation.buyprice

							if(stationBalance - (maxLitres * litrePrice) > 0) then
								-- station has enough balance

								if(maxLitres > 0) then
									TriggerServerEvent('igs:buyFuel', nearStation.id, maxLitres, (maxLitres * litrePrice))
									TriggerServerEvent('igs:removeGas', lastPlate, maxLitres)
									selling = false
									ESX.ShowNotification('~g~Du hast ' .. maxLitres .. 'l für $' .. (maxLitres * litrePrice) .. ' verkauft!')
								else
									ESX.ShowNotification('~r~Dein Tank ist leer!')
									selling = false
								end
							else
								-- math to get the amount the station can buy

								maxMultiplier = 0
								for i = 0, maxLitres do
									if((nearStation.account - (i * litrePrice)) > 0) then
										-- this is okay.
										maxMultiplier = i
									else
										break
								...
								if(maxMultiplier > 0) then
									TriggerServerEvent('igs:buyFuel', nearStation.id, maxMultiplier, (maxMultiplier * litrePrice))
									TriggerServerEvent('igs:removeGas', lastPlate, maxMultiplier)
									selling = false
									ESX.ShowNotification('~g~Du hast ' .. maxMultiplier .. 'l für $' .. (maxMultiplier * litrePrice) .. ' verkauft!')
								else
									ESX.ShowNotification('~r~Diese Tankstelle hat nicht genug Geld!')
									selling = false
								end
							end
					...
					if(nearStation.fuel > -1 and nearStation.owner ~= nil) then
						DrawText3Ds(pumpCoords.x, pumpCoords.y, pumpCoords.z + 1.2, Config.Strings.ExitVehicle)
						useable = true
					elseif(nearStation.fuel <= 0)  then
						DrawText3Ds(pumpCoords.x, pumpCoords.y, pumpCoords.z + 1.2, "Diese Tankstelle hat keinen Benzin mehr")
						useable = false
					else
						DrawText3Ds(pumpCoords.x, pumpCoords.y, pumpCoords.z + 1.2, "Diese Tankstelle wurde noch nicht verkauft")
						useable = false
					end
				end
				...
				if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped), vehicleCoords) < 2.5 and useable then
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								DrawText3Ds(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, "Drücke ~E~ um für $" .. nearStation.fuelcost .. "/l zu tanken!")
